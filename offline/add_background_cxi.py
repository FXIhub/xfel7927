...
for run in tqdm(args.run) :
    ...
    args.vds_file      = PREFIX+'scratch/vds/r%.4d.cxi' %run
    args.events_file   = PREFIX+'scratch/events/r%.4d_events.h5'%run
    args.back_file     = PREFIX+'scratch/powder/r%.4d_powder_is_hit_False_per_pixel.h5'%run
    ...
    print(f'{run=} {back_counts=}', file=sys.stderr)
